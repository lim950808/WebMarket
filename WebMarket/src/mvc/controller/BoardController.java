...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		if(command.equals("/BoardListAction.do")) {//등록된 게시글 목록 페이지 출력 요청
			...
			//로그인 아이디가 없으면 로그인 페이지로 이동처리
			if(sessionId==null || "".equals(sessionId)) {
				response.sendRedirect("./member/loginMember.jsp");
				return;
			}
			//로그인 후 게시글 등록 페이지로 이동했는지, 로그인한 작성자 이름 얻기
	...
	private void requestBoardView(HttpServletRequest request) {
		...
		//파라미터로 넘어온 글 번호와 페이지 번호(리스트로 다시 이동시 해당 페이지 블럭으로 이동처리하기 위해)
		int num = Integer.parseInt(request.getParameter("num"));
		int pageNum = Integer.parseInt(request.getParameter("pageNum"));
		
		//개별 속성 변수를 묶어서 처리할 DTO 객체 생성
		BoardDTO board = new BoardDTO();
		//DAO에 상세 글 번호와 페이지 번호를 넘겨서 DB로부터 얻은 글 정보를 다시 받음.
		board = dao.getBoardByNum(num, pageNum);
		
		//상세 글정보를 상세페이지로 전달하기 위해 request에 세팅
		request.setAttribute("num", num); //글 번호 - autoBoxing(기본타입-래퍼객체로 자동 형변환)
	...
	private void requestBoardList(HttpServletRequest request) {
		...
		if(total_record%limit == 0) { //예) 487건 -> 한페이지당 5건 출력 => 487=5*97+0, 97페이지
		...
		//페이지 세그먼트 처리
		int startPage = ((pageNum-1)/10)*10+1; //1페이지 1, 2페이지 1, 6페이지 1, 11페이지 11, 19페이지 11, 20페이지 11, 21페이지
	...
	private void requestLoginName(HttpServletRequest request) {
		//파라미터로 넘어온 request의 id에 해당하는 값 얻기
 	   	...
 	   	BoardDAO dao = BoardDAO.getInstance();
 	   	String name = dao.getLoginNameById(id); //id에 해당하는 name얻기 메소드
